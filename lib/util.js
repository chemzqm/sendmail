...
mailParser.prototype.parse = function(cb){
  async.series({
      ...
      if(err) cb(err);
        ...
        res.attachment.push({date: marked(res.text), alternative: true});
        cb(null, res);
      }
    }.bind(this));
};
...
mailParser.prototype.parseAttachment = function(cb){
  ...
  if(/^attachment:/.test(line)){
    this.lines.shift();
    var files = line.replace(/^attachment/,'').trim().splite(',');
...
mailParser.prototype.parseContent = function(cb){
  var content_lines = [], line = this.lines[0];
  if(!/^html:/.test(line)){
...
mailParser.prototype.parseTo = function(cb){
  ...
  if(!/^to:/.test(line)) throw new Error("can not find field to");
  else {
    this.lines.shift();
    line = line.replace(/^to:\s*/,"");
    names = line.trim().split(',');
    if(names.length === 0 ) cb(new Error("no receiver specified"));
